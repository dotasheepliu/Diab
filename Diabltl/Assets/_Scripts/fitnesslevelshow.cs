...
public class fitnesslevelshow : MonoBehaviour {
	...
	void Start () {
		...
		text.GetComponent<Text> ().text = "Fitness " + (200f / 2f).ToString () + "%"; 
	}

	public void sugarlevelminusone () {
		offsetMax = GetComponent<RectTransform> ().offsetMax;
		GetComponent<RectTransform> ().offsetMax = new Vector2 (offsetMax.x - 1f, offsetMax.y);
		text.GetComponent<Text> ().text = "Fitness " + ((200f + GetComponent<RectTransform> ().offsetMax.x) / 2f).ToString () + "%"; 
	}
}
